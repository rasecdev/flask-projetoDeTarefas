...
class TarefaDetail(Resource):
    ...
    def put(self, id):
        ...
        parameters:
          ...
          - in: body
            name: Tarefa
            description: Modifica nova tarefa existente
            schema:
              type: object
              required:
                - titulo
        ...
        if tarefa_bd is None:
            ...
            if validate:
                ...
                result = tarefa_service.listar_tarefa_id(id)
                return make_response(ts.jsonify(result), 200)

    def delete(self, id):
        """
                 Remove a tarefa que possui o ID passado como parâmetro
                ---
                parameters:
                  - in: path
                    name: id
